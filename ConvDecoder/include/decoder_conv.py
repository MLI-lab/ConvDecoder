...
class conv_model(nn.Module):
    def __init__(self, num_layers, strides, pads, num_channels, num_output_channels, hidden_size, upsample_mode, act_fun,sig=None, bn_affine=True, skips=False,intermeds=None,bias=False,):
        ...
        for i in range(num_layers-1):
            ...
            if i == num_layers - 2:
                net1.add(nn.Conv2d(num_channels, num_channels, 3, strides[i], padding=pads[i], bias=bias))
            else:
                net1.add(nn.Conv2d(num_channels, num_channels, 3, strides[i], padding=pads[i], bias=bias))
            cntr += 1
        ...
        net2.add(nn.BatchNorm2d( nic, affine=bn_affine))
...
def convdecoder(
    ...
    model = conv_model(num_layers, strides, pads, num_channels, num_output_channels, hidden_size,
